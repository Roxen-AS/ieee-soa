...
/* ── DOMAIN CARD — expand uses inline hov state, NOT CSS hover ── */
export function DomainCard({ code,title,body,tip,delay=0 }:{code:string;title:string;body:string;tip?:string;delay?:number}) {
  ...
  return (
    <div
      ...
      style={{
        padding:"28px 24px",
        border:`1px solid ${hov?"rgba(200,255,0,0.25)":"var(--card-border)"}`,
        borderRadius:12,
        background:"var(--card-bg)",
        transform:hov?"translateY(-5px)":"none",
        boxShadow:hov?"0 18px 48px rgba(0,0,0,0.2),0 0 0 1px rgba(200,255,0,0.07)":"none",
        transition:"all .35s cubic-bezier(.22,1,.36,1)",
        position:"relative",overflow:"hidden",backdropFilter:"blur(8px)",cursor:"pointer",
      }}
    >
      {/* Top accent bar */}
      ...
      {/* Corner glow dot */}
      <div style={{position:"absolute",top:16,right:16,width:6,height:6,borderRadius:"50%",background:"var(--a1)",opacity:hov?0.9:0,boxShadow:hov?"0 0 14px var(--a1)":"none",transition:"opacity .3s"}}/>
      {/* Code */}
      ...
      {/* Body — controlled purely by inline hov state, no CSS class */}
      <div style={{
        maxHeight: hov ? "80px" : "0",
        overflow:"hidden",
        opacity: hov ? 1 : 0,
      ...
      {/* Tooltip */}
      {tip&&hov&&(
        <div style={{position:"absolute",bottom:"calc(100% - 10px)",left:"50%",transform:"translateX(-50%)",background:"var(--s1)",border:"1px solid var(--ln2)",padding:"7px 14px",borderRadius:4,fontFamily:"Space Mono,monospace",fontSize:9,color:"var(--ink2)",whiteSpace:"nowrap",zIndex:10}}>
          {tip}
        </div>
      )}
    </div>
  ...
  return (
    <div className="ticker-wrap" style={{overflow:"hidden",borderBottom:"1px solid var(--ln)",background:"var(--ticker-bg)",padding:"10px 0",transition:"background .4s"}}>
      <div className="ticker-inner" style={{display:"flex",whiteSpace:"nowrap",animation:"tick 30s linear infinite"}}>
        ...
        {doubled.map((item,i)=>(
          <span key={i}
            style={{display:"inline-flex",alignItems:"center",gap:24,fontFamily:"Oxanium,monospace",fontSize:10,fontWeight:600,color:"var(--ink3)",letterSpacing:3,textTransform:"uppercase",padding:"0 24px",transition:"color .2s",cursor:"default"}}
    ...
    <footer style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px clamp(20px,5vw,48px)",borderTop:"1px solid var(--ln)",background:"var(--foot-bg)",flexWrap:"wrap",gap:12,position:"relative",zIndex:10,transition:"background .4s,border-color .4s"}}>
      ...
      <div style={{display:"flex",gap:20}}>
        {["Privacy","IEEE.org","GitHub"].map(l=>(
          <span key={l}
            style={{fontFamily:"Space Mono,monospace",fontSize:9,color:"var(--ink3)",cursor:"pointer",transition:"color .2s"}}
            onMouseEnter={e=>(e.currentTarget.style.color="var(--a1)")}
            onMouseLeave={e=>(e.currentTarget.style.color="var(--ink3)")}
          >{l}</span>
        ))}
  ...
  return (
    <ToastCtx.Provider value={{show}}>
      ...
      <div style={{
        ...
        background:"var(--s1)",
        border:`1px solid ${type==="error"?"var(--a2)":"var(--a1)"}`,
        padding:"14px 20px",fontFamily:"Space Mono,monospace",fontSize:10,
        color:type==="error"?"var(--a2)":"var(--a1)",
